...
活動地點：第二電腦教室
  ...
  },
  {
    ...
    hint: "容器巢狀拼裝心法：\n1. 第一行放置 <!DOCTYPE html> 宣告；\n2. 放置 <html> 根容器；\n3. 將 <head> 與 <body> 拖入 <html> 容器內部；\n4. 將 <title> 放入 <head> 內部，<h1> 與 <p> 放入 <body> 內部！",
...
<img src="https://images.unsplash.com/photo-1517694712202-14dd9538aa97?w=300&auto=format&fit=crop&q=60" alt="特工工作筆電" class="rounded-lg shadow-md max-w-[240px] my-2" />`,
  ...
  },
  {
    ...
    hint: "表單巢狀與單選互斥心法：\n1. 輸入框放在最上方，設定 placeholder 提示文字；\n2. 將單選按鈕 <input type=\"radio\"> 拖入對應的 <label> 容器內部（包裹起來）；\n3. 兩個 radio 的 name 屬性皆設定相同值（如 shift），瀏覽器才會產生單選互斥！",
    defaultMode: "container",
    requiredBlocksCount: 5,
...
<label>
  ...
  },
  {
    ...
    hint: "綜合表單拼裝心法：\n1. 頂部 <div> 姓名區塊內部放入姓名輸入框（含 placeholder=\"王XX\"）；\n2. 依序建立早、中、晚三個 <label>，並將對應的 radio 按鈕拖入其 label 內部；\n3. 三個 radio 的 name 皆為 'time' 達成互斥；\n4. 最後在底部放入「送出」按鈕！",
    defaultMode: "container",
